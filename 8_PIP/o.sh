...
if [ "$#" -eq 0 ]; then
    ...
    case "$1" in
        ...
        00)
            ...
            /opt/python/cp38-cp38/bin/python setup.py bdist_wheel --python-tag cp38
            /opt/python/cp38-cp38/bin/python -m auditwheel repair dist/*.whl -w dist/
            rm -f ../../15_PIP_MAIN/purecpp_libs/dist/*cp38*.whl
            cp ./dist/*cp38*.whl ../../15_PIP_MAIN/purecpp_libs/dist/
            cd ..
            ;;
        1)
        ...
        11)
            ...
            /opt/python/cp38-cp38/bin/python setup.py bdist_wheel --python-tag cp38
            /opt/python/cp38-cp38/bin/python -m auditwheel repair dist/*.whl -w dist/
            rm -f ../../15_PIP_MAIN/purecpp_extract/dist/*cp38*.whl
            cp ./dist/*cp38*.whl ../../15_PIP_MAIN/purecpp_extract/dist/
            cd ..
            ;;
        ...
        22)
            ...
            /opt/python/cp38-cp38/bin/python setup.py bdist_wheel --python-tag cp38
            /opt/python/cp38-cp38/bin/python -m auditwheel repair dist/*.whl -w dist/ --exclude libtorch_cpu.so 
            rm -f ../../15_PIP_MAIN/purecpp_chunks_clean/dist/*cp38*.whl
            cp ./dist/*cp38*.whl ../../15_PIP_MAIN/purecpp_chunks_clean/dist/
            cd ..
            ;;
        4)
        ...
        44)
            ...
            /opt/python/cp38-cp38/bin/python setup.py bdist_wheel --python-tag cp38
            /opt/python/cp38-cp38/bin/python -m auditwheel repair dist/*.whl -w dist/
            rm -f ../../15_PIP_MAIN/purecpp_meta/dist/*cp38*.whl
            cp ./dist/*cp38*.whl ../../15_PIP_MAIN/purecpp_meta/dist/        
            cd ..
            ;;
        5)
        ...
        55)
            ...
            /opt/python/cp38-cp38/bin/python setup.py bdist_wheel --python-tag cp38
            /opt/python/cp38-cp38/bin/python -m auditwheel repair dist/*.whl -w dist/
            rm -f ../../15_PIP_MAIN/purecpp_embed/dist/*cp38*.whl
            cp ./dist/*cp38*.whl ../../15_PIP_MAIN/purecpp_embed/dist/
            cd ..
            ;;
        9)
            echo "--------------------------------------------------------------------------"
        ...
        9)
            ...
            /opt/python/cp38-cp38/bin/python setup.py bdist_wheel --python-tag cp38
            /opt/python/cp38-cp38/bin/python -m auditwheel repair dist/*.whl -w dist/ --exclude libtorch_cpu.so 
            rm -f ../../15_PIP_MAIN/purecpp/dist/*cp38*.whl
            cp ./dist/*cp38*.whl ../../15_PIP_MAIN/purecpp/dist/
            cd ..
            ;;
        10)
            echo "Removendo apenas os diretórios de purecpp.."
            ...
            ;;
        *)
            echo "Parâmetro inválido. Use 0, 00, 1, 11, 2, 22, 4, 44, 5, 55, 6, 7, 8, 9"
            echo "0)Removendo apenas os diretórios de purecpp_libs..."
            echo "00) Removendo apenas os diretórios de purecpp_libs e gerando o wheel..."
            echo "1)Removendo apenas os diretórios de purecpp_extract..."
            echo "11) Removendo apenas os diretórios de purecpp_extract e gerando o wheel..."
